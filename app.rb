...
def get_pulls
	...
	repos_pulls.each do |repo_pulls|
		...
		filtered_pulls = pulls.find_all {|pull| (Date.today - Date.parse(pull["updated_at"])).to_i <= 30}
...
def get_issues
	...
	repos_issues.each do |repo_issues|
		...
		filtered_issues = issues.find_all {|issue| (Date.today - Date.parse(issue["updated_at"])).to_i <= 30}
...
def get_commits
	...
	repos.each do |repo|
		...
		if commits.kind_of?(Array)
			filtered_commits = commits.find_all {|commit| (Date.today - Date.parse(commit["commit"]["author"]["date"])).to_i <= 30}
